...
成员清单
atoms.ts: contentAtom(书摘正文与元数据；shareDate=全局共享摘录日期，null 跟随今天，全版式同源)、DEFAULT_CARD_WIDTH(卡片默认宽度常量，Stage 的「恢复默认宽度」按钮与 styleAtom 初值共用同一份)、styleAtom(版式/schemeByLayout 每版式配色/font 字体/quoteFontSize 正文字号 16|18|20|22/quoteFontWeight 正文粗细 400|500|600 全局统一且迁移时白名单收敛/图片背景/有背景/framePadding 外框留白/卡片宽度，以及八项跨版式共享显隐；章节仍受书名级联)、exportAtom(倍率/格式)、avatarAtom(头像库)。styleAtom 升 key excerpt.style.v12，并按 v11→v10 顺序迁移旧外观配置；content 为会话态。
